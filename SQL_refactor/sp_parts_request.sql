...
/******************
Create sp_select_all_parts_request_details Stored Procedure
***************/
...
/******************
Create sp_deactivate_request_by_id Stored Procedure
***************/
...
AS
    BEGIN
        UPDATE [dbo].[Parts_Request] 
        SET [Is_Active] = 0 
        WHERE @Parts_Request_Id = [Parts_Request_ID]
    END
GO

/******************
Create sp_approve_request Stored Procedure
***************/
-- Initial Creator: Parker Svoboda
...
AS
    BEGIN
		...
		BEGIN TRY
			SELECT @Quantity_Requested = pr.[Qty_Requested], 
                @Parts_Inventory_ID = pr.[Parts_Inventory_ID], 
                @Line_Item_Description = iv.[Item_Description], 
                @Part_Name = iv.[Part_Name]
			FROM [dbo].[Parts_Request_Line_Items] pr
			INNER JOIN [dbo].[Parts_Inventory] iv 
                ON iv.[Parts_Inventory_ID] = pr.[Parts_Inventory_ID]
			WHERE pr.[Parts_Request_ID] = @Parts_Request_ID;

			UPDATE [dbo].[Parts_Inventory] 
            SET [Ordered_Qty] += @Quantity_Requested 
            ...
            VALUES 
                (@Purchase_Order_ID, @Parts_Inventory_ID, @LineNumber, @Part_Name, @Quantity_Requested, @Price, @Line_Item_Description);
			COMMIT TRANSACTION;
		END TRY
		BEGIN CATCH
			IF @@TRANCOUNT > 0
